...
_SYSTEM_PROMPT = (
    ...
    "Your ONLY job when you see an image is: "
    "1) Look for any question, problem, or query written on the screen. "
    "2) If you find one, answer it with JUST the direct answer — "
    "no explanation, no steps, no extra words. Only the answer. "
    "3) If the screen has multiple questions, answer all of them briefly. "
    "4) If there is NO question on the screen, describe what you see in 1-2 sentences. "
    "Address the user as 'Yuvan'. Always respond in English only. "
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            self._player.write_log(f"IRA: {full}")
                            print(f"[Vision] 💬 {full}")
                    transcript = []
...
def analyze_prepared_image(image_bytes: bytes, mime_type: str,
                           user_text: str = "", player=None) -> bool:
    """
    Analyse an ALREADY-captured image (e.g. a screenshot taken by the UI with
    IRA's own window hidden). Same vision session as screen_process, but skips
    the capture step so the caller controls exactly what is in the frame.
    """
    if not image_bytes:
        return False
    try:
        ...
        print(f"[Vision] ❌ Could not start session: {e}")
        return False
    _session.analyze(image_bytes, mime_type,
                     user_text or "Describe what is on the screen.")
    return True
