...
def _ask_for_url(prompt_text: str = "YouTube video URL:") -> str | None:
    try:
        ...
        url = simpledialog.askstring("IRA", prompt_text, parent=root)
...
def _summarize_with_gemini(transcript: str, video_url: str) -> str:
    # General (non-coding) task → runs on Gemini directly.
    # (Nara / Bluesminds are reserved for coding tasks only.)
    ...
    response  = _client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=(
                "You are IRA, an AI assistant. "
                "Summarize YouTube video transcripts clearly and concisely. "
                "Structure: 1-sentence overview, then 3-5 key points. "
                "Be direct. Address the user as 'Yuvan'. "
...
def _handle_play(parameters: dict, player) -> str:
    ...
    if not query:
        return "Please tell me what you'd like to watch, Yuvan."
...
def _handle_summarize(parameters: dict, player, speak) -> str:
    ...
    if not url:
        return "No URL provided, Yuvan. Summary cancelled."
    if not _is_valid_youtube_url(url):
        return "That doesn't appear to be a valid YouTube URL, Yuvan."

    video_id = _extract_video_id(url)
    if not video_id:
        return "Could not extract video ID from that URL, Yuvan."

    if player:
        player.write_log(f"[YouTube] Summarizing: {url}")
    if speak:
        speak("Fetching the transcript now, Yuvan. One moment.")

    transcript = _get_transcript(video_id)
    if not transcript:
        return "I couldn't retrieve a transcript for that video, Yuvan."
    ...
    try:
        ...
        return f"Summary generation failed, Yuvan: {e}"
...
def _handle_get_info(parameters: dict, player, speak) -> str:
    ...
    if not url or not _is_valid_youtube_url(url):
        return "Please provide a valid YouTube URL, Yuvan."

    video_id = _extract_video_id(url)
    if not video_id:
        return "Could not extract video ID, Yuvan."
    ...
    if not info:
        return "Could not retrieve video information, Yuvan."
    ...
    if speak:
        speak(f"Here's the video info, Yuvan. {result.replace(chr(10), '. ')}")
...
def _handle_trending(parameters: dict, player, speak) -> str:
    ...
    if not trending:
        return f"Could not fetch trending videos for region {region}, Yuvan."
    ...
    if speak:
        top3   = trending[:3]
        spoken = "Here are the top trending videos, Yuvan. " + ". ".join(
...
def _handle_search(parameters: dict, player) -> str:
    """Open YouTube search results page in the default browser."""
    query = parameters.get("query", "").strip()
    if not query:
        return "Please tell me what you would like to search for on YouTube, Yuvan."
    ...
    print(f"[YouTube] Opening search: {search_url}")
...
def youtube_video(
    ...
    try:
        ...
        return f"YouTube {action} failed, Yuvan: {e}"