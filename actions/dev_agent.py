...
try:
    ...
    PROJECTS_DIR = Path.home() / "Desktop" / "IRA Projects"
MAX_FIX_ATTEMPTS = 5
...
def _get_model(model_name: str):
    # Planner/coder text calls route through the Nara -> Bluesminds -> Gemini
    # fallback chain (see core/llm_client.py). `model_name` kept for
    # backward compatibility; the active provider/model comes from config.
...
def _build_project(
    ...
    try:
        ...
        msg = "Rate limit reached, Yuvan. Please try again in a moment."
        if speak: speak(msg)
        return msg
    ...
    if not file_codes:
        msg = "I could not write any project files, Yuvan."
        if speak: speak(msg)
        return msg
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        if not _has_error(last_output, run_command):
            msg = (
                f"Project '{proj_name}' is working, Yuvan. "
    ...
    msg = (
        f"I couldn't fully fix '{proj_name}' after {MAX_FIX_ATTEMPTS} attempts, Yuvan. "
...
def dev_agent(
    ...
    if not description:
        return "Please describe the project you want me to build, Yuvan."
