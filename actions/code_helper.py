...
def _get_gemini(model: str = GEMINI_MODEL):
    # Text code-gen/edit calls route through the Nara -> Bluesminds -> Gemini
    # fallback chain (see core/llm_client.py). The `model` arg is kept for
    # backward compatibility but the provider/model is chosen by the config.
    from core.llm_client import make_model
    return make_model()
...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    if not description:
        return "Please describe what you want me to build, Yuvan."
    ...
    msg = (
        f"I was unable to build a working version after {MAX_BUILD_ATTEMPTS} attempts, Yuvan. "
...
def _write_action(description, language, output_path, player) -> str:
    if not description:
        return "Please describe what you want me to write, Yuvan."
    if player:
...
def _edit_action(file_path, instruction, player) -> str:
    if not file_path:
        return "Please provide a file path to edit, Yuvan."
    if not instruction:
        return "Please describe what change to make, Yuvan."
...
def _explain_action(file_path, code, player) -> str:
    if file_path and not code:
        ...
        if err:
            return err
    if not code:
        return "Please provide code or a file path to explain, Yuvan."
...
def _run_action(file_path, args, timeout, player) -> str:
    if not file_path:
        return "Please provide a file path to run, Yuvan."
    p = Path(file_path)
    if not p.exists():
        return f"File not found: {file_path}"
    if player:
...
def _optimize_action(file_path, code, language, output_path, player) -> str:
    ...
    if file_path and not code:
        ...
        if err:
            return err
    if not code:
        return "Please provide code or a file path to optimize, Yuvan."
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    ...
    if not screenshot_path:
        return "Could not take screenshot, Yuvan. Please make sure PyAutoGUI is installed."
