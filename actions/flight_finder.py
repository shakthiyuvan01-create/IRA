...
from core.config import is_windows, is_mac, is_linux
...
def _format_spoken(
    ...
    if not flights:
        return (
            ...
            f"on {date}, Yuvan. The page may not have loaded correctly."
        )

    lines = [f"Here are the top flights from {origin} to {destination} on {date}, Yuvan."]
...
def flight_finder(parameters: dict, player=None, speak=None) -> str:
    ...
    if not origin or not destination:
        return "Please provide both origin and destination, Yuvan."
    if not date_raw:
        return "Please provide a departure date, Yuvan."
    ...
    if speak:
        speak(f"Searching flights from {origin} to {destination} on {date}, Yuvan.")
    ...
    try:
        ...
        if not raw_text:
            return "Could not retrieve flight data, Yuvan. The page may not have loaded."

        if speak:
            speak("Analysing the results now, Yuvan.")
        ...
        return f"Flight search failed, Yuvan: {e}"