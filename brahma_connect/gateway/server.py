...
def _default_config_path(base_dir: Path) -> Path:
    return Path(base_dir) / "core" / "config" / "brahma_connect.json"
...
class BrahmaGatewayConfig:
    ...
    port: int = 8765
    enabled: bool = True
    admin_token: str = ""        # IRA: REQUIRED when host is exposed on the LAN
    advertise: bool = True
...
class BrahmaGateway:
    ...
    def _authorized(self, request: Request) -> bool:
        """Loopback is trusted (local admin surface); LAN exposure requires the
        configured admin_token — fail closed, never serve /gateway/* on LAN
        without one."""
        host = str(self.config.host or "").strip()
        if host in ("127.0.0.1", "localhost", "::1"):
            return True
        token = str(self.config.admin_token or "")
        if not token:
            return False
        header = str(request.headers.get("authorization", ""))
        return header == f"Bearer {token}" or request.query_params.get("token") == token
    ...
    def _build_app(self) -> FastAPI:
        ...
        async def gateway_auth(request: Request, call_next):
            if request.url.path.startswith("/gateway/") and not self._authorized(request):
    ...
    async def serve(self) -> None:
        if not self.config.enabled:
            return
        host = str(self.config.host or "").strip()
        if host not in ("127.0.0.1", "localhost", "::1") and not str(self.config.admin_token or "").strip():
            print(f"[IRA Connect] REFUSED to start: LAN exposure (host={host}) requires "
                  f"admin_token in core/config/brahma_connect.json", flush=True)
            self._append_log("GATEWAY_REFUSED", host=host, reason="lan_without_admin_token")
            return
        self._running = True
        advertised = False
