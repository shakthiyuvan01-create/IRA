...
class BrahmaConnectService:
    ...
    def route_command(self, target: str, action: str, parameters: dict[str, Any] | None = None) -> dict[str, Any]:
        # IMPORTANT: never call asyncio.run() here. asyncio.run() spins up a new
        # event loop and CLOSES it on return. If this method runs on the caller's
        # already-running loop (e.g. the live bot loop, via an action executed on
        # the bot's executor), that close tears down the shared default
        # ThreadPoolExecutor, after which every asyncio.to_thread()/run_in_executor()
        # raises "cannot schedule new futures after shutdown" — permanently freezing
        # the session into a reconnect livelock (see core/main.py run() loop).
        # Fix: dispatch onto the gateway's own background loop when available,
        # otherwise use a loop scoped to THIS call only (off the running loop).
        coro = self.gateway.route_command(target, action, parameters or {})
