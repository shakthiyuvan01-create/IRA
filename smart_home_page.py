...
BG = "#050505"                     # C.BG
PANEL = "#0C0E11"                  # C.PANEL2
PANEL_2 = "#101214"                # C.GLASS
ACCENT = "#00AFFF"                 # C.BLUE
ACCENT_SOFT = "rgba(0, 175, 255, 0.12)"
ACCENT_LINE = "rgba(0, 175, 255, 0.22)"
TEXT = "#FFFFFF"                    # C.TEXT
TEXT_MED = "#B8BDC5"                # C.TEXT_MED
TEXT_DIM = "#5E646B"                # C.TEXT_DIM
GREEN = "#00FF88"                  # C.GREEN
...
def _soft_button_style() -> str:
    return (
        ...
        f"QPushButton:hover {{ border: 1px solid {ACCENT}; background: rgba(0, 175, 255,0.08); }}"
    )
...
class AddDeviceDialog(QDialog):
    def __init__(self, service: SmartHomeService, parent=None):
        ...
        title.setFont(QFont("Segoe UI Variable Text", 18, QFont.Weight.Black))
    ...
    def _card(self, title: str) -> QFrame:
        ...
        lay.setSpacing(6)
        lbl = QLabel(title)
        lbl.setFont(QFont("Segoe UI Variable Text", 11, QFont.Weight.Bold))
        lbl.setStyleSheet(f"color: {TEXT};")
        lay.addWidget(lbl)
        return frame

_BRAND_ICON = {
    "atomberg": "fan", "kasa": "plug", "hue": "lightbulb", "lg": "tv",
    "daikin": "snowflake", "tuya": "settings", "nest": "home", "smartthings": "radio",
}
_BRAND_COLOR = {
    "atomberg": "#00FF88", "kasa": "#00b4d8", "hue": "#ffa500", "lg": "#a500a5",
    "daikin": "#7bd3f7", "tuya": "#ff5a5f", "nest": "#FFFFFF", "smartthings": "#0064ff",
}
_KIND_ICON = {"fan": "fan", "light": "lightbulb", "ac": "snowflake", "tv": "tv",
              "plug": "plug", "sensor": "radio", "curtain": "layout", "vacuum": "zap"}
_KIND_COLOR = {"fan": "#00FF88", "light": "#ffa500", "ac": "#7bd3f7", "tv": "#00b4d8"}


def _brand_pixmap(name: str, color: str, size: int):
    from core.ui import icon_pm
    return icon_pm(name, color, size, 1.8)


def _kind_pixmap(kind: str, size: int = 18):
    return _brand_pixmap(_KIND_ICON.get(kind, "settings"),
                         _KIND_COLOR.get(kind, "#B8BDC5"), size)


def _svg_img(name: str, color: str, size: int) -> str:
    """Inline-SVG <img> tag for rich-text labels (no unicode glyphs)."""
    import base64
    from core.ui import _ICON_SVG
    ...
    return (f'<img src="data:image/svg+xml;base64,{b64}" width="{size}" '
            f'height="{size}" style="vertical-align: middle;">')


def _dot_svg(size: int = 9, color: str = "#E0A13C") -> str:
    import base64
    r = size / 2
    svg = (f'<svg xmlns="http://www.w3.org/2000/svg" width="{size}" height="{size}">'
           f'<circle cx="{r}" cy="{r}" r="{r - 1}" fill="{color}"/></svg>')
    b64 = base64.b64encode(svg.encode("utf-8")).decode("ascii")
    return (f'<img src="data:image/svg+xml;base64,{b64}" width="{size}" '
            f'height="{size}" style="vertical-align: middle;">')
    ...
    def _build_platform_page(self) -> QWidget:
        ...
        for idx, platform in enumerate(self._platforms()):
            ...
            badge.setPixmap(_brand_pixmap(_BRAND_ICON.get(platform.key, "settings"),
                                          _BRAND_COLOR.get(platform.key, "#B8BDC5"), 20))
            
            badge_style = {
                "atomberg": "background: rgba(0, 255, 136, 0.08); color: #00FF88; border: 1px solid rgba(0, 255, 136, 0.2); border-radius: 18px;",
            ...
            p_name.setFont(QFont("Segoe UI Variable Text", 10, QFont.Weight.Bold))
            ...
            p_sub.setFont(QFont("Segoe UI Variable Text", 8))
    ...
    def _build_scan_page(self) -> QWidget:
        ...
        self._scan_label.setFont(QFont("Segoe UI Variable Text", 12, QFont.Weight.Bold))
...
class _DeviceTile(ClickableFrame):
    ...
    def __init__(self, device: dict[str, Any], parent=None):
        ...
        self.power_btn.setFont(QFont("Segoe UI Variable Text", 10))
        ...
        if Path(img_path).exists():
            ...
            self.img_lbl.setPixmap(_kind_pixmap(kind, 30))
        lay.addWidget(self.img_lbl)
        
        # Device details
        self.name_lbl = QLabel(str(device.get("name", "Device")))
        self.name_lbl.setFont(QFont("Segoe UI Variable Text", 9, QFont.Weight.Bold))
        self.name_lbl.setStyleSheet("color: #ffffff;")
        
        self.sub_lbl = QLabel(str(device.get("manufacturer", "")))
        self.sub_lbl.setFont(QFont("Segoe UI Variable Text", 8))
    ...
    def _build_controls(self):
        ...
        if kind == "fan":
            ...
            self.val_lbl.setFont(QFont("Segoe UI Variable Text", 8))
            ...
            for s in range(1, 6):
                ...
                if is_on and s <= current_speed:
                    seg.setStyleSheet("background: #00AFFF; border-radius: 2.5px;")
                else:
            ...
            on_lbl.setFont(QFont("Segoe UI Variable Text", 8, QFont.Weight.Bold))
            ...
            if is_on:
                ...
                vol_lbl.setFont(QFont("Segoe UI Variable Text", 8))
            ...
            temp_lbl.setFont(QFont("Segoe UI Variable Text", 12, QFont.Weight.Bold))
            ...
            if is_on:
                ...
                cool_lbl = QLabel(_svg_img("snowflake", "#B8BDC5", 12) + " Cool")
                cool_lbl.setFont(QFont("Segoe UI Variable Text", 8))
                cool_lbl.setStyleSheet("color: rgba(255, 255, 255, 0.7); background: rgba(255, 255, 255, 0.05); border-radius: 4px; padding: 2px 6px;")
                
                fan_lbl = QLabel(_svg_img("fan", "#B8BDC5", 12) + " Fan Auto")
                fan_lbl.setFont(QFont("Segoe UI Variable Text", 8))
            ...
            self.val_lbl.setFont(QFont("Segoe UI Variable Text", 8, QFont.Weight.Bold))
    ...
    def set_active(self, active: bool):
        if active:
            self.setStyleSheet(f"""
                QFrame#DeviceTileFrame {{
                    background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #101214, stop:1 #0C0E11);
                    border: 1.5px solid {ACCENT};
                    border-radius: 18px;
                }}
            """)
            ...
            self.setStyleSheet(f"""
                QFrame#DeviceTileFrame {{
                    background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #0C0E11, stop:1 #050505);
                    border: 1px solid rgba(255, 255, 255, 0.06);
                    border-radius: 18px;
                }}
                QFrame#DeviceTileFrame:hover {{
                    background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #14181F, stop:1 #0C0E11);
        ...
        is_on = bool(self.device.get("is_on"))
        if is_on:
            self.power_btn.setStyleSheet("QPushButton { color: #00FF88; background: rgba(0, 255, 136, 0.08); border: 1px solid rgba(0, 255, 136, 0.25); border-radius: 15px; } QPushButton:hover { background: rgba(0, 255, 136, 0.15); }")
        else:
...
class SmartHomePage(QWidget):
    def __init__(self, parent=None):
        ...
        self.setObjectName("SmartHomePage")
        self.setStyleSheet(f"QWidget#IRAHomePageModern {{ background: transparent; }} QScrollArea {{ background: transparent; border: none; }}")
        ...
        act_title.setFont(QFont("Segoe UI Variable Text", 11, QFont.Weight.Bold))
        ...
        self._view_all_act_btn.setStyleSheet(f"QPushButton {{ background: transparent; color: {ACCENT}; border: none; font-size: 11px; font-weight: bold; text-align: left; padding-left: 4px; }} QPushButton:hover {{ color: #E05555; }}")
    ...
    def _section_card(self, title: str) -> QFrame:
        ...
        lay.setSpacing(10)
        head = QLabel(title)
        head.setFont(QFont("Segoe UI Variable Text", 12, QFont.Weight.Bold))
    ...
    def _build_header(self, lay: QVBoxLayout):
        ...
        title.setFont(QFont("Segoe UI Variable Text", 24, QFont.Weight.Black))
        title.setStyleSheet(f"color: {TEXT}; letter-spacing: 1px;")
        subtitle = QLabel("Control your smart home with IRA Echo.")
        subtitle.setFont(QFont("Segoe UI Variable Text", 11))
        ...
        self._voice_button.setStyleSheet(f"QPushButton {{ background: rgba(255, 255, 255, 0.02); color: {TEXT}; border: 1px solid rgba(255,255,255,0.06); border-radius: 12px; font-size: 16px; }} QPushButton:hover {{ border: 1px solid {ACCENT}; background: rgba(0, 175, 255,0.08); }}")
    ...
    def _build_voice_card(self, lay: QVBoxLayout):
        ...
        self._mic_orb.setFont(QFont("Segoe UI Variable Text", 26))
        self._mic_orb.setStyleSheet(f"QLabel {{ background: rgba(0, 175, 255,0.12); color: {TEXT}; border: 2px solid rgba(0, 175, 255,0.55); border-radius: 43px; }}")
        vf.addWidget(self._mic_orb)
        middle = QVBoxLayout()
        self._voice_state_lbl = QLabel("Listening...")
        self._voice_state_lbl.setFont(QFont("Segoe UI Variable Text", 18, QFont.Weight.Bold))
        ...
        for _ in range(36):
            ...
            bar.setStyleSheet("background: rgba(0, 175, 255,0.20); border-radius: 2px;")
    ...
    def _build_my_devices(self, lay: QVBoxLayout):
        ...
        title.setFont(QFont("Segoe UI Variable Text", 13, QFont.Weight.Bold))
        title.setStyleSheet(f"color: {TEXT}; letter-spacing: 1px;")
        ...
        self._add_btn.setStyleSheet(f"QPushButton {{ background: transparent; color: {TEXT}; border: 1px solid {ACCENT}; border-radius: 12px; padding: 0 14px; }} QPushButton:hover {{ background: rgba(0, 175, 255,0.10); }}")
        ...
        empty_title.setFont(QFont("Segoe UI Variable Text", 18, QFont.Weight.Bold))
        ...
        self._empty_add_btn.setStyleSheet(f"QPushButton {{ background: rgba(0, 175, 255,0.12); color: {TEXT}; border: 1px solid {ACCENT}; border-radius: 12px; }} QPushButton:hover {{ background: rgba(0, 175, 255,0.18); }}")
    ...
    def _build_voice_control_card(self) -> QFrame:
        ...
        title.setFont(QFont("Segoe UI Variable Text", 10, QFont.Weight.Bold))
        title.setStyleSheet(f"color: {TEXT}; letter-spacing: 1px;")
        ...
        word = QLabel("Hey IRA")
        word.setFont(QFont("Segoe UI Variable Text", 10, QFont.Weight.Bold))
        ...
        mic_btn = QPushButton()
        mic_btn.setIcon(QIcon(_brand_pixmap("mic", "#FFFFFF", 18)))
    ...
    def _build_drawer(self) -> QFrame:
        ...
        title.setFont(QFont("Segoe UI Variable Text", 12, QFont.Weight.Bold))
        ...
        self._drawer_name.setFont(QFont("Segoe UI Variable Text", 18, QFont.Weight.Bold))
    ...
    def _open_drawer(self, device: dict[str, Any]):
        ...
        self._drawer_meta.setText(f"""
            <div style='line-height: 140%;'>
                ...
                <span style='color: rgba(255,255,255,0.45); font-size: 10pt;'>Connection:</span> <span style='color: #00FF88; font-weight: bold; font-size: 10pt;'>Local</span><br>
        ...
        is_on = bool(device.get("is_on"))
        state_color = "#00FF88" if is_on else "rgba(255,255,255,0.4)"
    ...
    def _refresh(self):
        ...
        if devices:
            status_html = (
                ...
                "<span style='color: #00FF88; font-size: 12px; font-weight: bold;'>●</span> "
    ...
    def _refresh_activity(self):
        ...
        for row in self._activity_items:
            ...
            time_lbl.setFont(QFont("Segoe UI Variable Text", 9))
            ...
            badge = QLabel()
            badge.setPixmap(_kind_pixmap(kind, 14))
            badge.setFixedSize(32, 32)
            badge.setAlignment(Qt.AlignmentFlag.AlignCenter)
            badge.setFont(QFont("Segoe UI Variable Text", 11))
            
            badge_style = {
                "fan": "background: rgba(0, 255, 136, 0.08); color: #00FF88; border: 1px solid rgba(0, 255, 136, 0.25); border-radius: 16px;",
            ...
            detail.setFont(QFont("Segoe UI Variable Text", 9))
    ...
    def _animate_voice(self):
        ...
        if hasattr(self, "_wave_bars") and self._wave_bars:
            for idx, bar in enumerate(self._wave_bars):
                ...
                bar.setStyleSheet(f"background: {'rgba(0, 175, 255,0.55)' if self._voice_state in ('Listening', 'Executing') else 'rgba(255,255,255,0.18)'}; border-radius: 2px;")
        if hasattr(self, "_voice_small_bars") and self._voice_small_bars:
            for idx, bar in enumerate(self._voice_small_bars):
                ...
                bar.setStyleSheet(f"background: {'rgba(0, 175, 255,0.35)' if self._voice_state in ('Listening', 'Executing') else 'rgba(255,255,255,0.15)'}; border-radius: 2px;")
        if hasattr(self, "_voice_cmd_lbl") and self._voice_cmd_lbl:
            self._voice_cmd_lbl.setText({"Idle": '"Hey IRA"', "Listening": '"Turn bedroom fan to speed 4"', "Thinking": '"Understanding..."', "Executing": '"Applying command..."', "Completed": '"Done"'}.get(self._voice_state, '"Hey IRA"'))
        if hasattr(self, "_mic_orb") and self._mic_orb:
            self._mic_orb.setPixmap(_brand_pixmap("mic",
                "#00FF88" if self._voice_state in ("Listening", "Executing") else "#5E646B", 40))
